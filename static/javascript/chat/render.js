...
export function buildReactionPickerHtml() { const options = REACTION_KEYS.map((key) => `<button type="button" class="chat-reaction-option" data-reaction="${key}" aria-label="Réagir avec ${key.toLowerCase()}" title="${key.toLowerCase()}">${REACTION_EMOJI[key]}</button>`); return `<div class="chat-reaction-picker">${options.join("")}</div>`; }
...
export function fillMessageRow(row, message, isOwn) { const body = message.is_deleted ? "<em>Message supprimé</em>" : `${message.content ? `<div class="chat-bubble-text">${escapeHtml(message.content).replace(/\n/g, "<br>")}</div>` : ""}${(message.attachments || []).map(buildAttachmentHtml).join("")}`; const edited = message.is_edited && !message.is_deleted ? `<span class="chat-bubble-edited">· modifié</span>` : ""; const actions = isOwn && !message.is_deleted ? `<div class="chat-bubble-actions"><button type="button" class="chat-bubble-action" data-action="edit">Modifier</button><button type="button" class="chat-bubble-action" data-action="delete">Supprimer</button></div>` : ""; row.innerHTML = `<div class="chat-bubble ${message.is_deleted ? "deleted" : ""}">${body}${buildReactionPickerHtml()}</div>${!message.is_deleted ? buildReactionsHtml(message) : ""}<div class="chat-bubble-meta"><span>${formatTime(message.created_at)}</span>${edited}${buildTicksHtml(message)}</div>${actions}`; }
